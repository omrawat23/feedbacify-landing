    ...
    return (
      <Card className="max-w-xs">
        ...
        <CardContent className="p-2">
          <Badge variant="secondary" className="mb-1">Rating: {rating}</Badge>
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      setDimensions({
        ...
        height: isMobile ? Math.max(window.innerHeight * 0.8, 600) : Math.min(window.innerHeight * 0.7, 500)
      })
    }
      ...
      <CardContent className="p-0 sm:p-6">
        <ChartContainer config={chartConfig}>
          <ResponsiveContainer width="100%" height={dimensions.height}>
            {isMobile ? renderPieChart() : (
              <BarChart
                data={processedData}
                margin={{ top: 20, right: 30, left: 20, bottom: 60 }}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="user" interval={0} tick={renderCustomAxisTick} height={60} axisLine={{ stroke: 'hsl(var(--border))' }} />
                <YAxis
                  domain={[0, 5]}
                  ticks={[0, 1, 2, 3, 4, 5]}
